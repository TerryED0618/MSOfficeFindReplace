Function New-MSExcelFindReplaceTextFile {
	...
	Param(
		...
		ValueFromPipeline=$TRUE,
		ValueFromPipelineByPropertyName=$TRUE )]
		[String[]] $Include = ( '*.xlsx', '*.xlsm', '*.xlsb', '*.xltx', '*.xltm', '*.xls', '*.xlt', '*.xls', '*.xml', '*.xml', '*.xlam', '*.xla', '*.xlw', '*.xlr' ),
	...
	Import-CSV -Path $FindReplacePath |
		ForEach-Object{
			...
			# NOT VERIFIED if this test is required by Excel Range.Replace method.
	...
	# Set Excel application settings
	$excelApp.Visible = [Microsoft.Office.Core.MsoTriState]::msoFalse
	Write-Debug "`$excelApp.DisplayAlerts:,$($excelApp.DisplayAlerts)"
	$excelApp.DisplayAlerts = $FALSE
	
	Get-ChildItem @getChildItemParameters |
		ForEach-Object {
		
			# Open Microsoft Excel document read-only
			# [Microsoft.Office.Interop.Excel.Workbooks]
			$document = $excelApp.Workbooks.Open( $PSItem.FullName, 0, [Microsoft.Office.Interop.Excel.XlFileAccess]::xlReadOnly ) # Filename, [Microsoft.Office.Interop.Excel.XlUpdateLinks] UpdateLinks, ReadOnly
			...
			If ( Update-MSExcelFindReplaceTextDocument -Document $document -FindReplaceTable $findReplaceTable ) {
			
				# Replacements executed, save document.
				#$document.Close( [Microsoft.Office.Interop.Excel.XlSaveAction]::xlSaveChanges, $outFilePathName ) # SaveChanges, Filename
				...
				$document.Close( [Microsoft.Office.Interop.Excel.XlSaveAction]::xlDoNotSaveChanges ) # SaveChanges
	...
	# Close Microsoft Excel application.  
	$excelApp.Quit()
	
	# Free up memory.  
	$NULL = [System.Runtime.InteropServices.Marshal]::ReleaseComObject([System.__ComObject]$excelApp)
	[gc]::Collect()
	...
	If ( (Test-Path -PathType Leaf -Path $outFilePathName) -And $MailFrom -And $MailTo -And $MailServer ) {
		...
		$messageSubject = "New Microsoft Excel replace text $reportType for $($outFilePathBase.ExecutionSourceName) on $((Get-Date).ToString('s'))"
